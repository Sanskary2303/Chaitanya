...
// Test command execution after system prompt update
function testAfterPromptUpdate() {
    const ws = new WebSocket('ws://localhost:8000');
    
    const testCommands = [
        'execute command ls -la data',
        'run command pwd',
        'list the files in data directory using ls command',
        'I need you to execute: ls -la'
    ];
    ...
    ws.on('open', function open() {
        ...
        console.log('🔄 Testing after system prompt update...\n');
        runNextTest();
    });
    ...
    function runNextTest() {
        ...
        const message = {
            ...
            clientId: 'test-updated-prompt-' + Date.now()
        };
            ...
            if (response.eventtype === 'stream.start') {
                ...
                if (fullResponse.includes('docData.json') || 
                    ...
                    console.log('✅ SUCCESS: Command executed successfully!');
                           ...
                           fullResponse.includes('I do not have a tool')) {
...
console.log('🧪 Testing command execution after system prompt update...');
testAfterPromptUpdate();
